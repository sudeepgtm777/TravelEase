...
function extractTourFeatures(tour) {
  ...
  if (tour.startLocation && tour.startLocation.description) {
    ...
    const locationKeywords = ['kathmandu', 'pokhara', 'chitwan', 'lumbini', 'everest', 'annapurna'];
  ...
  if (tour.summary || tour.name) {
    ...
    const tourTypes = [
      'trek', 'trekking', 'hiking', 'adventure', 'cultural', 'heritage',
      'nature', 'wildlife', 'safari', 'temple', 'monastery', 'spiritual',
      'mountain', 'hill', 'lake', 'national park', 'jungle'
    ];
...
async function getUserPreferences(userId) {
  ...
  for (const tourId of bookedTourIds) {
    ...
    const cleanTourId = typeof tourId === 'object' ? tourId._id || tourId.toString() : tourId.toString();
...
async function recommendToursByContent(userId, limit = 10) {
  ...
  if (userPreferences.size === 0) {
    ...
    return popularTours.map(tour => ({ tour, similarity: 0, reason: 'Popular tour' }));
  }
      ...
      for (const tourId of recommendedTourIds.slice(0, 2)) {
        ...
        if (tour) {
          collaborativeRecommendations.push({
            ...
            reason: `Users with similar taste also booked this (${Math.round(similarUser.similarity * 100)}% similarity)`
          });
        }
      }
    }
