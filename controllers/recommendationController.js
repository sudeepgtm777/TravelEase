...
exports.getRecommendations = catchAsync(async (req, res, next) => {
  ...
  const collaborativeRecommendations = await getCollaborativeRecommendations(userId, 4);
  ...
  res.status(200).json({
    ...
    data: {
      ...
      totalRecommendations: contentRecommendations.length + collaborativeRecommendations.length
    }
  });
});
...
exports.getRecommendationsPage = catchAsync(async (req, res, next) => {
  if (!req.user) {
    return next(new AppError('You must be logged in to view recommendations', 401));
  }
  ...
  const allRecommendations = [
    ...
    ...collaborativeRecommendations.map(r => ({ ...r, type: 'collaborative' }))
  ];
  ...
  if (allRecommendations.length === 0) {
    ...
    allRecommendations.push(...popularTours.map(tour => ({
      tour,
      similarity: 0,
      reason: 'Popular among travelers',
      type: 'popular'
...
exports.getUserBookingStats = catchAsync(async (req, res, next) => {
  ...
  bookingHistory.forEach(tour => {
    ...
    difficultyStats[tour.difficulty] = (difficultyStats[tour.difficulty] || 0) + 1;
